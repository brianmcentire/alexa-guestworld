...
NEXT_MONTH_THRESHOLD_DAY = int(os.getenv("E2E_NEXT_MONTH_THRESHOLD_DAY", "22"))
...
def _now_eastern_day():
    return datetime.now(ZoneInfo("America/New_York")).day


def _is_too_early_for_strict_next_month_assertions():
    return _now_eastern_day() < NEXT_MONTH_THRESHOLD_DAY


def _has_no_next_month_message(response):
    rl = response.lower()
    return ("don't know next month" in rl) or ("don't have next month" in rl)


def _assert_next_month_or_allow_early(response):
    if _has_no_next_month_message(response):
        assert _is_too_early_for_strict_next_month_assertions(), (
            "Expected next-month-capable answer, but got next-month-unavailable text "
            "on/after threshold day %d. Response: %s"
        ) % (NEXT_MONTH_THRESHOLD_DAY, response)
...
class TestWorldOnDateIntent:
    ...
    def test_ride_on_day_of_week(self, locale):
        ...
        responses = run_dialog(locale, ["ask which world where can i ride on sunday"])
    ...
    def test_ride_day_of_week_without_on(self, locale):
        ...
        responses = run_dialog(locale, ["ask which world where can i ride sunday"])
        assert len(responses) >= 1
        r = responses[0].lower()
        assert "guest worlds will be" in r
        assert "didn't catch which world" not in r
    ...
    def test_what_available_on_saturday(self, locale):
        responses = run_dialog(
            locale, ["ask which world what is available on saturday"]
        )
        assert len(responses) >= 1
        r = responses[0].lower()
    ...
    def test_weekend_query(self, locale):
        responses = run_dialog(locale, ["ask which world what can i ride this weekend"])
        assert len(responses) >= 1
        r = responses[0].lower()
        assert "guest worlds will be" in r or "saturday" in r
        assert "didn't catch which world" not in r
...
class TestTodaysWorldIntent:
    ...
    def test_ride_today(self, locale):
        responses = run_dialog(locale, ["ask which world where can i ride today"])
        assert len(responses) >= 1
    ...
    def test_bare_what_can_i_ride(self, locale):
        ...
        responses = run_dialog(locale, ["ask which world what can i ride"])
        assert len(responses) >= 1
        assert "todays guest worlds are" in responses[0].lower()

    def test_what_can_i_ride_now(self, locale):
        responses = run_dialog(locale, ["ask which world what can i ride now"])
        assert len(responses) >= 1
        assert "todays guest worlds are" in responses[0].lower()

    def test_what_is_available(self, locale):
        responses = run_dialog(locale, ["ask which world what is available"])
        assert len(responses) >= 1
        assert "todays guest worlds are" in responses[0].lower()
...
class TestTomorrowsWorldIntent:
    def test_ride_tomorrow(self, locale):
        responses = run_dialog(locale, ["ask which world where can i ride tomorrow"])
        assert len(responses) >= 1
        r = responses[0].lower()
        assert "tomorrow" in r or "don't know next month" in r

    def test_what_about_tomorrow(self, locale):
        responses = run_dialog(locale, ["ask which world what about tomorrow"])
        assert len(responses) >= 1
        r = responses[0].lower()
        assert "tomorrow" in r or "don't know next month" in r
...
class TestWhenWorldIntent:
    def test_when_is_london(self, locale):
        responses = run_dialog(locale, ["ask which world when is london"])
        assert len(responses) >= 1
        r = responses[0].lower()
        assert "london" in r
        assert "didn't catch which world" not in r
    ...
    def test_watopia_always_available(self, locale):
        responses = run_dialog(locale, ["ask which world when is watopia"])
        assert len(responses) >= 1
        assert "every day" in responses[0].lower()

    def test_when_is_yorkshire(self, locale):
        responses = run_dialog(locale, ["ask which world when is yorkshire"])
        assert len(responses) >= 1
        r = responses[0].lower()
...
class TestNextWorldIntent:
    def test_what_is_next(self, locale):
        responses = run_dialog(locale, ["ask which world what is next"])
        assert len(responses) >= 1
        r = responses[0].lower()
        assert any(
            phrase in r
...
# ---------------------------------------------------------------------------
# Next-month capability checks (date-aware)
# ---------------------------------------------------------------------------


@skip_no_ask
class TestNextMonthAwareness:
    """Validate next-month world handling, with threshold-based strictness."""

    def test_tomorrow_prefers_real_answer_when_next_month_available(self, locale):
        responses = run_dialog(locale, ["ask which world what about tomorrow"])
        assert len(responses) >= 1
        r = responses[0]
        _assert_no_errors(r)
        _assert_next_month_or_allow_early(r)

    def test_next_world_prefers_real_answer_when_next_month_available(self, locale):
        responses = run_dialog(locale, ["ask which world what is next"])
        assert len(responses) >= 1
        r = responses[0]
        _assert_no_errors(r)
    ...
    def test_after_that_prefers_real_answer_when_next_month_available(self, locale):
        responses = run_dialog(
            locale,
            [
                "ask which world what about tomorrow",
    ...
    def test_recurring_day_uses_next_month_after_day_passes(self, locale):
        responses = run_dialog(locale, ["ask which world what can i ride on the 4th"])
        assert len(responses) >= 1
        r = responses[0]
        _assert_no_errors(r)

        # If asking after the 4th and we still get next-month-unavailable messaging,
        # allow only before threshold; enforce after threshold.
        day = _now_eastern_day()
        if day > 4:
            _assert_next_month_or_allow_early(r)
...
class TestCrossIntentSession:
    ...
    def test_full_session(self, locale):
        ...
        assert "todays guest worlds are" in responses[0].lower()
        assert (
            "tomorrow" in responses[1].lower() or "don't know" in responses[1].lower()
        )
...
class TestWeeklyChallengeIntent:
    ...
    def test_what_is_route_of_the_week(self, locale):
        responses = run_dialog(
            locale, ["ask which world what is the route of the week"]
        )
        assert len(responses) >= 1
        r = responses[0].lower()
        assert "route" in r
    ...
    def test_what_is_climb_of_the_week(self, locale):
        responses = run_dialog(
            locale, ["ask which world what is the climb of the week"]
        )
        assert len(responses) >= 1
        r = responses[0].lower()
        assert "climb" in r
    ...
    def test_weekly_challenges(self, locale):
        responses = run_dialog(
            locale, ["ask which world what are the weekly challenges"]
        )
        assert len(responses) >= 1
        r = responses[0].lower()
    ...
    def test_how_long_is_route(self, locale):
        responses = run_dialog(
            locale, ["ask which world how long is the route of the week"]
        )
        assert len(responses) >= 1
        r = responses[0].lower()
    ...
    def test_next_weeks_route(self, locale):
        responses = run_dialog(
            locale, ["ask which world what is next weeks route of the week"]
        )
        assert len(responses) >= 1
        r = responses[0].lower()
